...
const OnboardingExpenseModal: React.FC<EditAssignTaskModalProps> = ({ selectedType }) => {
    ...
    const handleClose = () => {
        if (selectedType === "operations") {
            dispatch(setIsAddOperationsExpense(false));
        } else {
            dispatch(setIsAddExpenseModal(false));
        }
    };
    ...
    const handleSubmit = async (values: ChecklistOverviewProps) => {
        ...
        try {
            if (selectedChecklistItem && selectedType) {
                await updatePharmacyAssignChecklist(dispatch, { assigned_id: selectedChecklistItem.assigned_id, checklist_id: selectedChecklistItem.checklist_id, ...payload }, selectedType);
            }
            handleClose();
        } catch (error: any) {
