        ...
        <div className="w-full mx-auto">
            {items.map((item, index) => (
                <div key={index} className="shadow-md rounded-xl mb-6 overflow-hidden">
                    <div
                        className={`flex justify-between items-center px-6 sm:py-2  md:py-4 cursor-pointer ${activeIndex === index ? 'bg-primary text-white' : 'bg-white'}`}
                        onClick={() => onTitleClick(index)}
                    >
                        <h1 className="text-sm sm:text-xs md:text-base lg:text-lg">{item.title}</h1>
                        ...
                        <div className="p-6 bg-white border-t border-gray-300">
                            {Array.isArray(item.content) && item.content.length > 0 ? (
                                    ...
                                    {item.content.map((text, idx) => (
                                        <li key={idx} className="flex items-center justify-between py-3">
                                            <div className="flex items-center space-x-2">
                                                ...
                                                <span className="text-xs lg:text-base md:text-sm sm:text-[9px]">{text}</span>
                                            </div>
                                ...
                                <div className="flex items-center justify-between">
                                    <p className="text-base lg:text-base md:text-sm sm:text-[9px]">{item.content}</p>
                                    <button className="text-gray-500 hover:text-blue-500 transition" onClick={handleEditQuestion}>
                                        <Image src="/edit-icon.svg" alt="" width={18} height={18} />
                                    </button>
                                </div>
