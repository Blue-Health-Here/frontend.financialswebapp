    ...
    return (
        <div className={`flex items-center justify-between bg-white px-4 py-2.5 border rounded-lg shadow-sm w-full ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
            ...
            <div className="flex space-x-2">
                <MdOutlineFileDownload className={`text-grey w-6 h-6 ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'}`} size={20} onClick={disabled ? undefined : fileDownload} />
                    <Image src="/delete-icon.svg" alt="delete-icon" className={`${disabled ? 'cursor-not-allowed' : 'cursor-pointer'}`} width={15} height={15} onClick={disabled ? undefined : handleDelete} />
            </div>
        </div>
    );
};
